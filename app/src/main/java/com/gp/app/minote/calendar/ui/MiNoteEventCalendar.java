...
public class MiNoteEventCalendar extends MiNoteCalendar implements
    ...
    private void attachClickListener(final CalendarGridView calendarGridView) {
        final TextView dayTextView = calendarGridView.getDayTextView();

        dayTextView.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                final int clickedDay = Integer.valueOf(dayTextView.getText().toString());

                final int clickedMonth = currentVisibleMonthCalendar.get(Calendar.MONTH) + 1;

                final int clickedYear = currentVisibleMonthCalendar.get(Calendar.YEAR);

                String clickedDate = MiNoteUtil.pad(clickedDay) + "/" + MiNoteUtil.pad(clickedMonth) + "/" + MiNoteUtil.pad(clickedYear);

                String todaysDate = MiNoteUtil.pad(todayDate) + "/" + MiNoteUtil.pad(todayDateMonth) + "/" + MiNoteUtil.pad(todayDateYear);

                final long clickedTimeInMillis = MiNoteUtil.parseDateAndTimeString(clickedDate, "dd/mm/yyyy");

                final long todaysTimeInMillis = MiNoteUtil.parseDateAndTimeString(todaysDate, "dd/mm/yyyy");

                if (clickedTimeInMillis >= todaysTimeInMillis) {
                    createPopUpMenuForDate(dayTextView, calendarGridView.getDay(), month + 1, year);
                }
            }
        });
    }
    ...
    private void initEventCalendar(CalendarGridView calendarGridView) {
        ...
        final String formattedDate = MiNoteUtil.pad(day) + "/" + MiNoteUtil.pad(month + 1) + "/" + MiNoteUtil.pad(year);
        ...
        if (numberOfEvents > 0) {
            ...
            eventTextView.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View v) {
                    final long currentGridTime = MiNoteUtil.parseDateAndTimeString(formattedDate, "dd/mm/yyyy");
                    ...
                    new EventModificationGUI(isReadOnly).createEventModificationList(context,
                            day, month + 1, year);
                }
            });
    ...
    private void createDialogForMenuItem(final int month, final int year, final int day, final PopupMenu popupMenu,
        ...
        if (item.getItemId() == R.id.createEvent) {
            ...
            new EventModificationGUI(false).createEventModificationList(context, day, month, year);
